...
clipboard copy/paste) -- see the RESEARCH_EMBED_TRACK_* PersistentConfig
toggles in config.py and src/lib/utils/researchEmbedTracking.ts for where
these get produced. Deliberately a separate table from `chat`, not folded
into the chat's own JSON blob: this is high-volume, append-only telemetry
with a different access pattern (bulk export for analysis) than a chat
transcript, and keeping it separate means a researcher who never enables
tracking never touches this table at all.
"""
...
class ResearchEmbedEventsTable:
    def insert_events(
        self, user_id: str, events: list[ResearchEmbedEventForm]
    ) -> int:
        """Bulk-inserts a batch of events for one participant. Returns the
    ...
    def get_all_events(
        ...
        """For admin export -- every event across every participant, oldest
        first. skip/limit are there so a very large study's export can be
        paginated instead of loading everything into memory at once."""
        with get_db() as db:
